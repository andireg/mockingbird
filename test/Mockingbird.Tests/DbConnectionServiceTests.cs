...
public class DbConnectionServiceTests
{
    ...
    public async Task TestQuery()
    {
        ...
        IEnumerable<DbConnectionItem> actual = await context.Instance.QueryAsync("KEY", TestContext.Current.CancellationToken);
    ...
    {
        using IMockContext<DbConnectionService> context = MockContextFactory.Start<DbConnectionService>();
        long actual = await context.Instance.ExecuteScalarAsync("KEY", TestContext.Current.CancellationToken);
        Assert.Equal(42, actual);Assert.Equal(42, actual);
        context.Verify();
    }
}